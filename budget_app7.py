...
class BudgetApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Gamified Financial Literacy App")
        self.root.geometry("1200x800")
        self.root.configure(bg='lightblue')

        self.username = ""
        self.avatar = tk.StringVar()

        self.expenses = []
        self.expense_dates = set()
        self.goals = {}
        self.reminders = []
    ...
    def setup_style(self):
        ...
        style.configure('TNotebook', background='white', tabposition='nw')  # Tabs at the top
        style.configure('TNotebook.Tab', background='green', foreground='white', padding=[10, 10], font=('Arial', 12, 'bold'))
        style.configure('Treeview', background='white', foreground='black', fieldbackground='white')
        style.configure('Treeview.Heading', background='green', foreground='white')
        style.configure('TButton', background='green', foreground='white', font=('Arial', 12, 'bold'))
    ...
    def create_welcome_screen(self):
        ...
        welcome_label = tk.Label(self.welcome_frame, text="Welcome to the Gamified Financial Literacy App!", font=('Arial', 24, 'bold'), bg='white', fg='green')
        welcome_label.pack(pady=20)

        self.username_var = tk.StringVar()

        form_frame = ttk.Frame(self.welcome_frame, style='TFrame')
        form_frame.pack(pady=20)

        tk.Label(form_frame, text="Enter your name:", bg='white', fg='green', font=('Arial', 14)).grid(row=0, column=0, padx=5, pady=5)
        tk.Entry(form_frame, textvariable=self.username_var, font=('Arial', 14), bg='white', fg='black').grid(row=0, column=1, padx=5, pady=5)

        tk.Label(form_frame, text="Choose an avatar:", bg='white', fg='green', font=('Arial', 14)).grid(row=1, column=0, padx=5, pady=5)
        avatars = ["Avatar 1", "Avatar 2", "Avatar 3"]
        self.avatar_combobox = ttk.Combobox(form_frame, textvariable=self.avatar, values=avatars, font=('Arial', 14))
        self.avatar_combobox.grid(row=1, column=1, padx=5, pady=5)

        start_button = tk.Button(form_frame, text="Start", command=self.start_app, bg='green', fg='white', font=('Arial', 14, 'bold'))
...
### Part 3: Spending Goals Tab
    ...
    def create_spending_goals_tab(self):
        ...
        for category, progressbar in self.progressbars.items():
            ttk.Label(self.spending_goals_frame, text=category, bg='white', fg='green', font=('Arial', 12)).pack()
...
### Part 6: Financial Literacy Tab
    ...
    def show_investing_info(self):
        ...
        **Basics of Investing for Teenagers:**
    ...
    def show_jobs_info(self):
        self.show_info_window("Jobs", """
        **Types of Jobs for Teenagers:**
        1. **Retail Jobs**: Working in stores and boutiques.
        2. **Food Service Jobs**: Working in cafes, restaurants, and fast food.
        3. **Tutoring Jobs**: Helping peers or younger students with their studies.
        4. **Internships**: Gaining experience in a professional setting.
        5. **Freelance Jobs**: Offering services like writing, graphic design, etc.

        **Tips for Job Search:**
        - **Build a Resume**: Importance of showcasing skills and experience.
        - **Networking**: Leveraging personal connections for job opportunities.
        - **Interview Preparation**: Tips for acing job interviews.
        - **Balancing Work and School**: Managing time effectively.
        """, self.show_jobs_quiz)
    ...
    def show_jobs_quiz(self):
        self.show_quiz_window("Jobs Quiz", [
            ("What is a benefit of internships?", ["Gaining professional experience", "Getting rich quickly", "Avoiding school"], 0),
            ("Why is networking important?", ["For building job opportunities", "For socializing", "For avoiding work"], 0)
        ])
